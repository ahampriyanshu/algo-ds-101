...
bool DLList::isEmpty() { return header->getNext() == header; }
...
void DLList::sortByName() { sortByName(getFirstPos(), getLastPos()); }
...
void DLList::sortByDepartment(DLNode *leftP, DLNode *rightP) {
  ...
  while (i != j) {
    while (i != j and
           i->getData().getDepart() <= pivot->getData().getDepart()) {
      i = i->getNext();
    }
    while (i != j and
           j->getData().getDepart() >= pivot->getData().getDepart()) {
      j = j->getPrev();
    }
    if (i != j) {
      exchange(i, j);
