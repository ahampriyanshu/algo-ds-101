...
SLNode *SLList::findData(Client &e) {
  ...
  do {
    ...
    aux = aux->getNext();
  } while (aux != anchor) return nullptr;
}

void SLList::deleteAll() {
  SLNode *aux = anchor;
  SLNode *mark = anchor do {
    aux = anchor;
    anchor = anchor->getNext() delete aux;
  }
  while (anchor != mark)
    ;
...
SLList::~SLList() { deleteAll(); }

SLList::SLList() { anchor = nullptr; }

bool SLList::isEmpty() { return anchor == nullptr; }
...
void SLList::insertData(SLNode *p, Client &e) { // insertar despuès
  ...
  bool SLList::isValidPos(SLNode * &p) {
    ...
    SLNode *SLList::getFirsPos() { return anchor; }
    ...
    void SLList::writeToDisk(const std::string &fileName) {
      ...
      if (!myFile.is_open()) {
        throw ListException("No se pudo Abrir el Archivo" + fileName +
                            "Para escritura");
      }
      if (!isEmpty()) {
        SLNode *aux = anchor;
